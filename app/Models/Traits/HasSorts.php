    ...
    public function scopeSorts($query, $sort)
    {
        try{
            if(! property_exists($this, 'allowedSorts')){
                throw new \Exception('Por favor agrega la propiedad public $allowedSorts en '.get_class($this), 500);
            }
            ...
            foreach($sortFields as $field){
                ...
                if(! collect($this->allowedSorts)->contains($field)){
                    throw new \Exception('Parámetro inválido en sort, '.$field.' no fue encontrado', 400);
                }
